...
input_ ='''hcl:#6b5442 ecl:brn iyr:2019
...
for line in input_.split('\n\n'):
	...
	for x in ['byr','iyr','eyr','hgt','hcl','ecl','pid']:
		temp.append(x in line)
	l.append(str(all(temp)))
print(Counter(l)['True'])
...
for line in input_.split('\n\n'):
	...
	if all(temp):
		...
		for val in line.split(' '):
			chk = val.split(':')[1]
			if 'byr' in val:
				present_and_valids.append(int(chk) in [*range(1920,2002+1)] and len(chk) == 4)
			if 'iyr' in val:
				present_and_valids.append(int(chk) in [*range(2010,2020+1)] and len(chk) == 4)
			if 'eyr' in val:
				present_and_valids.append(int(chk) in [*range(2020,2030+1)] and len(chk) == 4)
			if 'hgt' in val:
				present_and_valids.append(('cm' in chk[-2:] and int(chk[:-2]) in [*range(150,193+1)]) or ('in' in chk[-2:] and int(chk[:-2]) in [*range(59,76+1)]))
			if 'hcl' in val:
				present_and_valids.append('#' in chk[0] and all([i in digits or i in ascii_lowercase for i in chk[1:]]))
			if 'ecl' in val:
				present_and_valids.append(chk in ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl' ,'oth'])
			if 'pid' in val:
				present_and_valids.append(len(chk) == 9)
		l.append(str(all(present_and_valids)))
print(Counter(l)['True'])